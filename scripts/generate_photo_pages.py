#!/usr/bin/env python3
"""Generate the frame listing on /photos/events from one table.
...
So the occasions live here instead, and the page is generated:
  ...
  * OCCASIONS is the whole content model, newest year first
...
ASSETS = ROOT / "assets" / "photos" / "events"

# The cover frame on /photos. Named here rather than hardcoded in the hub, so
# re-slugging a frame cannot leave the shelf pointing at a file that no longer
# exists: renaming 2023-kubecon-eu to -a did exactly that, and the hub 404'd
# while every gate still passed, because nothing checked it.
COVER = ("2023-kubecon-eu-a",
         "KubeCon + CloudNativeCon Europe 2023, cover frame of the Events collection.")
...
OCCASIONS = [
 ...
 ("2023", "Amsterdam, New York, Seattle, online",
   ...
   dict(
     ...
     title_en="Pi Day 2023", title_zh="2023 圆周率日",
...
def measured() -> dict[str, list[tuple[int, int]]]:
    """slug -> [(width, height), ...] widest first, read off the built files."""
    out: dict[str, list[tuple[int, int]]] = defaultdict(list)
    for f in sorted(ASSETS.glob("*.webp")):
        slug, _, _w = f.stem.rpartition("-")
        with Image.open(f) as im:
            out[slug].append((im.width, im.height))
    for slug in out:
        out[slug].sort(key=lambda wh: -wh[0])
    return out
...
def frame_html(slug, credit, alt, count, ind, F) -> str:
    variants = F.get(slug)
    if not variants:
        sys.exit(f"no built derivatives for {slug!r}. "
                 "Run scripts/build_photo_derivatives.py first.")
    base = f"/assets/photos/events/{slug}"
    w, h = variants[0]
    full = f"{base}-{w}.webp"
    pad = " " * (len(ind) + 9)
...
def occasion_html(o, ind, F) -> str:
    ...
    for frame in o["frames"]:
        ...
        out.append(frame_html(slug, credit, alt, n, ind + "      ", F))
    out += [f"{ind}   </div>", f"{ind}</article>"]
    return "\n".join(out)


def build_body(F) -> str:
    blocks = []
    for year, place_en, place_zh, occasions in OCCASIONS:
        n = sum(len(o["frames"]) for o in occasions)
        ...
        b = [f'{ind}<!-- {"=" * 60}',
             ...
             f'{ind}   </div>', ""]
        for o in occasions:
            b.append(occasion_html(o, ind + I, F))
            b.append("")
...
def render() -> tuple[str, str]:
    """Return the new text of the collection page and of the hub."""
    F = measured()
    body = build_body(F)

    used = {f[0] for _, _, _, occ in OCCASIONS for o in occ for f in o["frames"]}
    orphans = sorted(set(F) - used)
    if orphans:
        print(f"warning: built but unused: {', '.join(orphans)}", file=sys.stderr)

    frames = sum(len(o["frames"]) for _, _, _, occ in OCCASIONS for o in occ)
    occasions = sum(len(occ) for _, _, _, occ in OCCASIONS)
    years = [y for y, *_ in OCCASIONS]
    span = f"{min(years)}–{max(years)}"

    page = PAGE.read_text(encoding="utf-8")
    head, sep, rest = page.partition(BEGIN)
    if not sep:
        sys.exit(f"marker missing from {PAGE}: {BEGIN.strip()}")
    _old, sep2, tail = rest.partition(END)
    if not sep2:
        sys.exit(f"marker missing from {PAGE}: {END.strip()}")
    page = head + BEGIN + "\n\n" + body + "\n\n" + END + tail

    def census(label_en, value, text):
        return re.sub(
            rf'(<dt><span class="lang-en">{label_en}</span>.*?\n\s*<dd>)[^<]*(</dd>)',
            rf"\g<1>{value}\g<2>", text, flags=re.S)

    page = census("Frames", frames, page)
    page = census("Occasions", occasions, page)
    page = census("Years", span, page)

    hub = HUB.read_text(encoding="utf-8")

    cover_slug, cover_alt = COVER
    if cover_slug not in F:
        sys.exit(f"cover {cover_slug!r} has no built derivatives")
    cw, ch = min(F[cover_slug], key=lambda wh: wh[0])
    cover_img = (f'<img src="/assets/photos/events/{cover_slug}-{cw}.webp"\n'
                 f'                       width="{cw}" height="{ch}" loading="lazy" '
                 f'decoding="async"\n'
                 f'                       alt="{cover_alt}">')
    hub, n = re.subn(r'<img src="/assets/photos/events/[^>]*>', cover_img, hub)
    if n != 1:
        sys.exit(f"expected one cover image in {HUB.name}, found {n}")

    # Prose carries the range in two languages; keep both in step with the table.
    hub = re.sub(r'<span class="lang-en">\d+ frames · \d+ occasions · [^<]*</span>',
                 f'<span class="lang-en">{frames} frames · {occasions} occasions · {span}</span>',
                 hub)
    hub = re.sub(r'<span class="lang-zh" hidden>\d+ 张 · \d+ 个场合 · [^<]*</span>',
                 f'<span class="lang-zh" hidden>{frames} 张 · {occasions} 个场合 · {span}</span>',
                 hub)
    lo, hi = span.split("\u2013")
    for pat, rep in ((r"\b20\d\d to 20\d\d\b", f"{lo} to {hi}"),
                     (r"20\d\d 至 20\d\d", f"{lo} 至 {hi}")):
        page = re.sub(pat, rep, page)
        hub = re.sub(pat, rep, hub)
...
def main() -> int:
    ...
    page, hub = render()
    stale = [p.name for p, new in ((PAGE, page), (HUB, hub))
    ...
    frames = sum(len(o["frames"]) for _, _, _, occ in OCCASIONS for o in occ)
    occasions = sum(len(occ) for _, _, _, occ in OCCASIONS)
    print(f"wrote {PAGE.relative_to(ROOT)} and {HUB.relative_to(ROOT)}: "
          f"{frames} frames across {occasions} occasions")
    return 0
