...
So the occasions live here instead, and the page is generated:
  ...
  * HAND_BUILT lists collections whose page is written by hand and only needs a
    row on the hub; their tally is counted off the page, never typed
...
# ------------------------------------------------------------------
# Hand-built collections. A page this script does not own, but the hub still
# has to list. /photos/coffee/ is one immersive wall rather than a sheet of
# occasions: it carries its own stylesheet, its own staggered reveal and a
# square grid used nowhere else on the site, so driving it from an occasion
# table would mean teaching PAGE_TEMPLATE a layout that exists exactly once.
# Only the rack row lives here, and the tally is still counted off the page
# rather than typed, so the hub cannot claim a number the wall does not hold.
# ------------------------------------------------------------------
HAND_BUILT = [
    dict(
        slug="coffee",
        title_en="Coffee", title_zh="咖啡",
        # The frame the wall opens with, so the hub and the collection agree on
        # what the collection looks like. Already on the wall at 800x800, so
        # listing it costs no new asset; the rack crops it to 16/9 in CSS, as
        # it already does the 4:3 Teams cover.
        cover=("rose-petals-heart.jpg",
               "Heart latte art with rose petals on grey marble, cover frame "
               "of the Coffee collection."),
        # Cups, not frames and not occasions: the wall is one subject counted
        # by the mug, and there is no occasion table behind it to total.
        count_en="{n} cups · one wall", count_zh="{n} 杯 · 一整面墙",
        blurb_en="Latte art on marble, wood and black tables: hearts and "
                 "rosettas in paper, glass and porcelain, with matcha, hot "
                 "chocolate and egg coffee among them. A Victrola cup, a La "
                 "Marzocco cup, and the mug from the first Starbucks store at "
                 "Pike Place.",
        blurb_zh="大理石、木纹与黑色桌面上的拉花：纸杯、玻璃杯与瓷杯里的心形与叶形，"
                 "其间还有抹茶、热可可与蛋咖啡。Victrola 的杯子、La Marzocco 的杯子，"
                 "以及派克市场第一家星巴克的马克杯。",
    ),
]
...
def wall_count(slug: str) -> int:
    """Tiles on a hand-built collection page, counted off the page itself.

    The wall's own markup is the only source of truth for how many cups are on
    it, so the hub's tally is read from there rather than restated here. Swap a
    tile and the number does not move; add one and it does.
    """
    page = ROOT / "photos" / slug / "index.html"
    if not page.exists():
        sys.exit(f"hand-built collection {slug!r} has no page at "
                 f"{page.relative_to(ROOT)}")
    n = len(re.findall(r'<li style="--i:\d+">', page.read_text(encoding="utf-8")))
    if not n:
        sys.exit(f"no tiles found on the {slug!r} wall, so the hub would "
                 "advertise 0. Has the tile markup changed?")
    return n


def rack_html(rows) -> str:
    """The hub's collection rows, one per collection."""
    out = []
    for r in rows:
        out += [
            "            <li>",
            ...
            f'                  <img src="{r["cover"]}"',
            f'                       width="{r["cover_w"]}" height="{r["cover_h"]}" loading="lazy" decoding="async"',
            f'                       alt="{r["cover_alt"]}">',
            "               </figure>",
            "               <div>",
            '                  <p class="count">',
            f'                     <span class="lang-en">{r["count_en"]}</span>',
            f'                     <span class="lang-zh" hidden>{r["count_zh"]}</span>',
            "                  </p>",
            "                  <h2>",
            f'                     <a href="{r["href"]}">',
            f'                        <span class="lang-en">{r["title_en"]}</span>',
            f'                        <span class="lang-zh" hidden>{r["title_zh"]}</span>',
            "                     </a>",
            "                  </h2>",
            '                  <p class="blurb">',
            f'                     <span class="lang-en">{r["blurb_en"]}</span>',
            f'                     <span class="lang-zh" hidden>{r["blurb_zh"]}</span>',
            "                  </p>",
            '                  <span class="go">',
...
def render() -> dict[pathlib.Path, str]:
    ...
    for c in COLLECTIONS:
        ...
        rows.append(dict(
            href=f"/photos/{slug}/",
            cover=f"/assets/photos/{slug}/{cover_slug}-{cw}.webp",
            cover_w=cw, cover_h=ch, cover_alt=cover_alt,
            count_en=f"{frames} frames · {n_occ} occasions · {span}",
            count_zh=f"{frames} 张 · {n_occ} 个场合 · {span}",
            title_en=c["title_en"], title_zh=c["title_zh"],
            blurb_en=c["blurb_en"], blurb_zh=c["blurb_zh"],
        ))
    ...
    # Listed, not generated: the row is built here, the page is not touched.
    for c in HAND_BUILT:
        slug = c["slug"]
        n = wall_count(slug)
        cover_file, cover_alt = c["cover"]
        cover = ROOT / "assets" / "photos" / slug / cover_file
        if not cover.exists():
            sys.exit(f"cover {cover_file!r} missing for {slug!r}: "
                     f"{cover.relative_to(ROOT)}")
        with Image.open(cover) as im:
            cw, ch = im.width, im.height
        rows.append(dict(
            href=f"/photos/{slug}/",
            cover=f"/assets/photos/{slug}/{cover_file}",
            cover_w=cw, cover_h=ch, cover_alt=cover_alt,
            count_en=c["count_en"].format(n=n),
            count_zh=c["count_zh"].format(n=n),
            title_en=c["title_en"], title_zh=c["title_zh"],
            blurb_en=c["blurb_en"], blurb_zh=c["blurb_zh"],
        ))
